...
class Nested_set {
    ...
    public function __construct() {
        ...
        $this->db = & $CI->db;
        $this->setControlParams('Categories', 'lft', 'rgt', 'id', 'parent_id', 'name');
    }
    ...
    public function deleteNode($node) {
        if ($node[$this->primary_key_column_name] == 1)
            return FALSE;
    ...
    public function getChildOfTree() {
        return $this->getNodesWhere($this->parent_column_name . ' =  0 ');
    }
    ...
    function buildCrumbs($crumbData) {
        ...
        foreach ($crumbData as $itemId) {
            ...
            $retVal .= '<li>' . anchor(
                            'shop/category/' . $itemId['id'], $itemId[$this->text_column_name], array(
    ...
    public function getSubTree_int($node) {
        ...
        foreach ($tree_handle['result_array'] as $menuItem) {
            $menuData['items'][$menuItem['id']] = $menuItem;
            $menuData['parents'][$menuItem['parent_id']][] = (int) $menuItem['id'];
        }
        ...
        if (isset($menuData['parents'][$parentId])) {
            ...
            foreach ($menuData['parents'][$parentId] as $itemId) {
                ...
                $retVal .= '<li class="depth-' . $depth . '">' . anchor(
                                'shop/category/' . $menuData['items'][$itemId]['id'], $menuData['items'][$itemId][$this->text_column_name], array(
                            'class' => 'id-' . $itemId['id']
                                )
                );
     ...
     * Vu Nguyen
     */
    function Menu_Node_Bootstrap($node, $menuData, $class) {

        $retVal = '<ol class="' . $class['ol'] . '">';

        $retVal .= '<li class="' . $class['li'] . '" ' . 'data-id="' . $node[$this->primary_key_column_name] . '">' . '<div class="' . $class['div'] . '">' . $node[$this->text_column_name];

        if ($this->checkNodeHasChildren($node)) {

            $retVal = $this->Menu_Bootstrap($node[$this->parent_column_name], $menuData, $class);
        }
    ...
    function Menu_Bootstrap($parentId, $menuData, $class) {
        $retVal = '';

        if (isset($menuData['parents'][$parentId])) {
            $retVal = '<ol class="' . $class['ol'] . '" menu-id="' . $parentId . '">';
            ...
            foreach ($menuData['parents'][$parentId] as $itemId) {
                ...
                $retVal .= '<span style="float: right"><i class="icon-plus"></i><i class="icon-pencil"></i>';
                
                if($menuData['items'][$itemId]['id'] != 1) {
                    $retVal .= '<i class="icon-trash"></i>';
                }
                ...
                $retVal .= $this->Menu_Bootstrap($itemId, $menuData, $class);
    ...
    function Menu_Bootstrap_frontend($parentId, $menuData, $class) {
        $retVal = '';

        if (isset($menuData['parents'][$parentId])) {
            $retVal = '<ul id="nav" class="' . $class['ul'] . '" menu-id="' . $parentId . '">';
            //var_dump($menuData['parents'][$parentId]);die;
             foreach ($menuData['parents'][$parentId] as $itemId) {

               // var_dump($this->checkNodeHasChildren($menuData['items'][$itemId]));
                if($this->checkNodeHasChildren($menuData['items'][$itemId]) == true){
                    $retVal .= '<li class="' . $class['li'] . '" ' . 'data-lft="' . $menuData['items'][$itemId]['lft'] . '"' . 'data-rgt="' . $menuData['items'][$itemId]['rgt'] . '"' . 'data-id="' . $menuData['items'][$itemId]['id'] . '">';
                    $retVal .= '<a href="">'.$menuData['items'][$itemId][$this->text_column_name].'&darr;</a>';
                }
                else
                {
                    $retVal .= '<li class="" ' . 'data-lft="' . $menuData['items'][$itemId]['lft'] . '"' . 'data-rgt="' . $menuData['items'][$itemId]['rgt'] . '"' . 'data-id="' . $menuData['items'][$itemId]['id'] . '">';
                    $retVal .= '<a href="">'.$menuData['items'][$itemId][$this->text_column_name].'</a>';
                }
            ...
            //    // $retVal .= '</div>';
                 //$class1  = array('ul' => ,'sub','li'=>'');
                // $class1 = array('ul' => 'sub','li' => '');
                 $retVal .= $this->Menu_Bootstrap_frontend_x($itemId, $menuData, $class);
    ...
    function Menu_Bootstrap_frontend_x($parentId, $menuData, $class) {
        $retVal = '';

        if (isset($menuData['parents'][$parentId])) {
            $retVal = '<ul class="' . $class['ul'] . '" menu-id="' . $parentId . '">';
            //var_dump($menuData['parents'][$parentId]);die;
             foreach ($menuData['parents'][$parentId] as $itemId) {
                
                if($this->checkNodeHasChildren($menuData['items'][$itemId])){
                    $retVal .= '<li class="' . $class['li'] . '" ' . 'data-lft="' . $menuData['items'][$itemId]['lft'] . '"' . 'data-rgt="' . $menuData['items'][$itemId]['rgt'] . '"' . 'data-id="' . $menuData['items'][$itemId]['id'] . '">';
                    $retVal .= '<a href="">'.$menuData['items'][$itemId][$this->text_column_name].'&raquo;</a>';
                }
                else
                {
                    $retVal .= '<li class="" ' . 'data-lft="' . $menuData['items'][$itemId]['lft'] . '"' . 'data-rgt="' . $menuData['items'][$itemId]['rgt'] . '"' . 'data-id="' . $menuData['items'][$itemId]['id'] . '">';
            ...
            //    // $retVal .= '</div>';
                 //$class1  = array('ul' => ,'sub','li'=>'');
                 $class1 = array('ul' => 'sub','li' => '');
                 $retVal .= $this->Menu_Bootstrap_frontend_x($itemId, $menuData, $class1);
    ...
    function Item_List($parentId, $menuData) {
        $retVal = array();
        if (isset($menuData['parents'][$parentId])) {

            foreach ($menuData['parents'][$parentId] as $itemId) {

                $retVal[] = (int) $menuData['items'][$itemId]['id'];

                $retVal = array_merge($retVal, $this->Item_List($itemId, $menuData));
            }
        }
    ...
    function Item_List_Node($parentId, $menuData) {
        $retVal = array();
        ...
        if (isset($menuData['parents'][$parentId])) {
            ...
            foreach ($menuData['parents'][$parentId] as $itemId) {
                ...
                $retVal = array_merge($retVal, $this->Item_List_Node($itemId, $menuData));
            }
        }
        return $retVal;
    }
    ...
    public function build_menu($id) {
        $nodes = $this->getNodeFromId($id);
        $root_nodes = $this->getSubTree($nodes);
        $class = array('ol' => 'dd-list', 'li' => 'dd-item', 'div' => 'dd-handle');
        $menu = $this->Menu_Bootstrap($id, $root_nodes, $class); //var_dump($menu);die;
        return $menu;
    }

    public function build_menu_frontend($id) { 
        $nodes = $this->getNodeFromId($id);
        $root_nodes = $this->getSubTree($nodes);
        $class = array('ul' => '','li' => 'parent minus');
        $menu = $this->Menu_Bootstrap_frontend($id, $root_nodes, $class); //var_dump($menu);die;

        return $menu;
    }

    public function build_sub_menu() {
        $sub_menu = $this->getChildOfTree();
        if (empty($sub_menu))
            return FALSE;
        $sub = '';
        foreach ($sub_menu as $key => $value) {
            ...
            $sub .= '<div data-menu-id="' . $value['id'] . '" data-menu-left="' . $value['lft'] . '" data-menu-right="' . $value['rgt'] . '"class="dd3-content">' . $value['name'] . '</div>';
        }
        return $sub;
    }
    ...
    public function build_menu_category($id, $array, $IDMenu, $cateInMenu) {
    	$nodes = $this->getNodeFromId($id);
    	$root_nodes = $this->getSubTree($nodes);
    	$class = array('ol' => 'dd-list', 'li' => 'dd-item', 'div' => 'dd-handle');
    	$menu = $this->Menu_Bootstrap_category($root_nodes, $class, $array, $IDMenu, $cateInMenu); //var_dump($menu);die;
    	return $menu;
    }
    function Menu_Bootstrap_category($menuData, $class, $array, $IDMenu, $cateInMenu) {
    	$retVal = '';
    	if(!empty($array)){
    		$retVal = '<ol class="' . $class['ol'] . '" menu-id="' . $IDMenu . '">';
    		foreach ($cateInMenu as $k => $v) {
    			$retVal .= '<li class="' . $class['li'] . '" ' . 'data-lft="' . $v['lft'] . '"' . 'data-rgt="' . $v['rgt'] . '"' . 'data-id="' . $v['CateID'] . '">';
    			$retVal .= '<span style="float: right"><i class="icon-trash"></i>';
    			
    			
    			$retVal .='</span>' . '<div class="' . $class['div'] . '">' . $v['name'];
    			
    			$retVal .= '</div>';
    			
    			$retVal .= $this->Menu_Bootstrap_category2($v['CateID'], $menuData, $class, $array, $IDMenu);
    ...
    function Menu_Bootstrap_category2($parentId, $menuData, $class, $array, $IDMenu) {
    	$retVal = '';
