...
class MY_Model extends CI_Model {
    ...
    public function get($ids = FALSE, $single = FALSE) {
        ...
        $flag = $ids == FALSE || is_array($ids) ? FALSE : TRUE;
        ...
        // Return results
        $flag == FALSE || $this->db->limit(1);
        $method = $flag ? 'row_array' : ($single ? 'num_rows' : 'result_array');
        //var_dump($method) ;die;
    ...
    public function get_by($key, $val = FALSE, $orwhere = FALSE, $single = FALSE) {
        ...
        // Return results
        $method = $single ? 'num_rows' : 'result_array';
        return $this->db->get($this->table_name)->$method();
    }
     ...
     * @author Joost van Veen
     */
    public function save($data, $id = FALSE, $single = FALSE, $where = FALSE) {
        if ($id == FALSE) {
            // This is an insert
            if ($single == TRUE) {
                $id = $this->db->insert_batch($this->table_name, $data);
            } else {
...
//    }
    ...
    public function set_start($start = 0, $limit = PERPAGA) {
        !is_int($start) || !($start >= 0) || $this->db->limit($limit, $start);
    }
